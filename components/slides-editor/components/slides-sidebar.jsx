...
function LayoutsPanel({ activeLayout, onAddSlide }) {
  return (
    <div className="flex min-h-0 flex-1 flex-col">
      <p className="shrink-0 px-3 py-3 text-[11px] text-text-secondary">Click a layout to insert a new slide.</p>
      <div className="min-h-0 flex-1 overflow-y-auto px-2 pb-3 scrollbar-subtle">
        {layouts.map((layout) => (
          ...
          >
            <span className="grid h-9 w-12 shrink-0 place-items-center rounded border border-border bg-surface-card text-muted-foreground">
              <LayoutTemplate className="h-4 w-4" />
            </span>
            <span className="min-w-0">
              <span className="block truncate text-sm text-white">{layout.label}</span>
              {activeLayout === layout.id ? <span className="block text-[11px] text-text-secondary">Current layout</span> : null}
            </span>
          </button>
        ))}
      </div>
      ...
      {themeOptions.map((theme) => {
        ...
        return (
          <button
            ...
            className={cn(
              "mb-1 flex w-full items-center gap-3 rounded-md px-2 py-2 text-left transition-colors hover:bg-surface-active",
              isActive && "bg-surface-active",
            )}
          >
            <span className="flex h-9 w-12 shrink-0 items-center justify-center gap-1 rounded border border-border" style={{ background: theme.background }}>
            ...
            {isActive ? <span className="shrink-0 text-[11px] text-text-secondary">Current</span> : null}
          </button>
        );
      })}
    </div>
      ...
      {transitionOptions.map((transition) => {
        ...
        return (
          <button
            ...
            className={cn(
              "mb-1 flex w-full items-center justify-between rounded-md px-3 py-2 text-left text-sm transition-colors hover:bg-surface-active",
              isActive ? "bg-surface-active text-white" : "text-muted-foreground",
            )}
          >
            {transition}
            {isActive ? <span className="text-[11px] text-text-secondary">Current</span> : null}
          </button>
        );
      })}
    </div>
...
function SlideNotesPanel({ notes, onSetNotes }) {
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col p-3">
      <p className="mb-2 text-[11px] uppercase tracking-normal text-text-secondary">Notes for this slide</p>
      <textarea
        value={draft}
        onChange={(event) => setDraft(event.target.value)}
        onBlur={commitNotes}
        placeholder="Add speaker notes…"
        className="min-h-0 flex-1 resize-none rounded-md border border-border bg-surface-card p-2 text-sm text-white placeholder:text-text-secondary focus:border-border-strong focus:outline-none"
      />
    </div>
  );
}
...
export function SlidesSidebar({
  ...
  return (
    <aside className={cn("hidden shrink-0 border-l border-border bg-surface-subtle xl:flex", active ? "w-80" : "w-12")}>
      {active ? (
